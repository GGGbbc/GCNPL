...
def calculate_recall(pred, label):
...
def evaluate(model, dataset, dataloader):
    ...
    with torch.no_grad():
        ...
        #        f1_relation = Counter()
        #        recall_relation = Counter()
        #        prec_relation = Counter()
        #        for i in range(1, dataset.num_class):
        #            tp = tp_by_relation[i]
        #            fp = fp_by_relation[i]
        #            fn = fn_by_relation[i]
        #
        ...
        #            f1s = 2 * precisions * recalls / (precisions + recalls) if (precisions + recalls) > 0 else 0
        #
        #            f1_relation[i] = f1s
        #            recall_relation[i] = recalls
        #            prec_relation[i] = precisions
        # ---------------------------------------------------
        mi_f1, ma_f1 = f1_score(pred, all_labels, dataset.num_class, dataset.NA_NUM)

        # 调用函数实现
        accuracy = calculate_accuracy(pred, all_labels)
        recall = calculate_recall(pred, all_labels)

        return mi_f1, ma_f1, accuracy, recall
...
for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            # print (args)
            global_step += 1
            print('训练损失：', tr_loss / global_step, '当前最佳的微平均F1', mx_res)

    mi_f1, ma_f1, accuracy, recall = evaluate(model, val_dataset, val_dataloader)
...
mi_f1, _, accuracy, recall = evaluate(model, test_dataset, test_dataloader)
print('------------------------------------------------------------------')
print("  - F1值为（F1）：", mi_f1)
print("  - 准确率（P）：", accuracy)
print("  - 召回率（R）：", recall)
